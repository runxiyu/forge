...
func writeTree(ctx context.Context, repoPath string, entries []treeEntry) (string, error) {
	...
	if err := gitcmd.Run(ctx, []string{"GIT_DIR=" + repoPath}, &buf, &out, os.Stderr, "hash-object", "-w", "-t", "tree", "--stdin"); err != nil {
		return "", err
	}
...
func buildTreeRecursive(ctx context.Context, repoPath, baseTree string, updates map[string][]byte) (string, error) {
	...
	walk = func(prefix, sha string) error {
		var out bytes.Buffer
		if err := gitcmd.Run(ctx, []string{"GIT_DIR=" + repoPath}, nil, &out, os.Stderr, "cat-file", "tree", sha); err != nil {
			return err
		}
		data := out.Bytes()
		i := 0
