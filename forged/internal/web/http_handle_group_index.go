...
func (s *Server) httpHandleGroupIndex(writer http.ResponseWriter, request *http.Request, params map[string]any) {
	...
	if request.Method == http.MethodPost {
	 ...
	 WHERE id = $2`,
		...
		if err = gogit.InitBare(filePath, s.config.Hooks.Execs); err != nil {
	...
	for rows.Next() {
		...
		repoList = append(repoList, nameDesc{name, description})
	}
