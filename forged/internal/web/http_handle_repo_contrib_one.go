...
func (s *Server) httpHandleRepoContribOne(writer http.ResponseWriter, request *http.Request, params map[string]any) {
	...
	if srcRefHash, err = gogit.GetRefHash(repo, "branch", srcRefStr); err != nil {
	...
	if dstBranchStr == "" {
		dstBranchStr = "HEAD"
		dstBranchHash, err = gogit.GetRefHash(repo, "", "")
	} else {
		dstBranchHash, err = gogit.GetRefHash(repo, "branch", dstBranchStr)
	}
	if err != nil {
