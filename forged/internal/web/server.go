...
func (s *Server) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	default:
		...
		case "repos":
			var repoIndex int
			if groupPath, moduleName, repoIndex, err = repos.ExtractRepo(segments); err != nil {
				ErrorPage404(s.templates, writer, params)
				return
			}
			params["group_path"] = groupPath
			...
			if len(segments) > repoIndex+1 {
				switch segments[repoIndex+1] {
				case "info":
			...
			repoURLRoot := "/"
			for _, part := range segments[:repoIndex+1] {
			...
			repoFeature := segments[repoIndex+1]
			switch repoFeature {
			case "tree":
				if misc.AnyContain(segments[repoIndex+2:], "/") {
					ErrorPage400(s.templates, writer, params, "Repo tree paths may not contain slashes in any segments")
					return
				}
				if dirMode {
					params["rest"] = strings.Join(segments[repoIndex+2:], "/") + "/"
				} else {
					params["rest"] = strings.Join(segments[repoIndex+2:], "/")
				}
				if len(segments) < repoIndex+3 && misc.RedirectDir(writer, request) {
					return
				}
			...
			case "commit":
				...
				params["commit_id"] = segments[repoIndex+2]
			...
			case "contrib":
				...
				case repoIndex + 3:
					params["mr_id"] = segments[repoIndex+2]
